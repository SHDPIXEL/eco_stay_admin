...
const ListType = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const formattedResponse = parseResponse.map((room) => ({
          ...
          status_show: `available: ${JSON.parse(room.status).available}, booked: ${JSON.parse(room.status).booked}`, // Format the status as a string
        }));
  ...
  const columns = [
    ...
    { header: "single occupancy price(INR)", accessor: "single_base_price" },
    { header: "Double occupancy price(INR)", accessor: "double_base_price" },
    { header: "Rooms status", accessor: "status_show" },
    {header: "New single occupancy price(INR)", accessor: "single_new_price"},
    {header: "New Double occupancy price(INR)", accessor: "double_new_price"},
  ];
